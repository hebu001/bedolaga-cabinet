...
## Время и срок токенов

- `authClock.ts` получает время из HTTP `Date` ответов API и refresh, затем отсчитывает прошедшее время через `performance.now()`. Ручной перевод часов устройства не влияет на проверку access-токена.
- До первого корректного серверного времени клиент передаёт структурно корректный токен backend. При `401` остаётся существующее ограниченное обновление и повтор запроса; локальные часы не используются как запасной источник.
- Окончательная проверка подписи и срока токенов остаётся на backend. Проверка не зависит от диагностического логирования.

## Логирование в production

- Временная трассировка авторизации и отправка клиентских событий на `/cabinet/diagnostics` удалены.
- `logger.error` и `ErrorBoundary` сохраняют штатный вывод ошибок. Отладочные `log`, `warn`, `debug` через `utils/logger.ts` доступны только при разработке.
- JWT, refresh-токены и снимки пользовательских сессий не отправляются в диагностический журнал.
