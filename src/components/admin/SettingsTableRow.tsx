...
export function SettingsTableRow({
  ...
  return (
    <div
      className={cn(
        'group px-4 py-3 transition-colors hover:bg-apple-elevated',
        isModified && 'bg-apple-amber/[0.04]',
        !isLast && 'border-b border-apple-hairline',
        className,
      )}
    >
      <div
      ...
      >
        ...
        <div className={cn('min-w-0', !isLongValue && 'lg:flex-1')}>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-[13px] font-medium text-apple-ink">{displayName}</span>

            {isModified && (
              <span className="rounded-full bg-apple-amber/15 px-2.5 py-1 text-[11px] font-semibold leading-none text-apple-amber">
          ...
          <div className="mt-0.5">
            <code className="font-mono text-[11px] text-apple-faint">{setting.key}</code>
          </div>

          {/* Description for long values */}
          {isLongValue && description && (
            <p className="mt-1 text-xs leading-relaxed text-apple-mute">{description}</p>
          )}
        </div>
        ...
        >
          ...
          {isModified && !setting.read_only && (
            <button
              ...
              className="flex-shrink-0 rounded-lg p-1.5 text-apple-faint opacity-0 transition-all hover:bg-apple-elevated hover:text-apple-ink disabled:opacity-50 group-hover:opacity-100 max-lg:opacity-100"
