...
export default function SubscriptionPurchase() {
  ...
  // Pre-flight that hits purchase-tariff / submitPurchase and swallows the
  // expected 402 — the backend's job is to save the cart in Redis on 402 so
  // that the top-up webhook can auto-fulfill the renewal after payment.
  // Used as `onBeforeTopUp` on InsufficientBalancePrompt's "Пополнить" button.
  const preflightSaveTariffCart = useCallback(async () => {
    ...
    try {
      await subscriptionApi.purchaseTariff(selectedTariff.id, days, trafficGb);
    } catch {
      // Expected: 402 insufficient_funds. Backend has already saved the cart.
    }
  }, [
    selectedTariff,
  ...
  const preflightSaveClassicCart = useCallback(async () => {
    try {
      await subscriptionApi.submitPurchase(currentSelection, subscriptionId);
    } catch {
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="mt-6">
                          ...
                          {/* Insufficient balance prompt (replaces fork-only direct payment modal) */}
                          {purchaseOptions && !hasEnoughBalance && missingAmount > 0 && (
                            <InsufficientBalancePrompt
                              missingAmountKopeks={missingAmount}
                              compact
                              className="mt-4"
                              onBeforeTopUp={preflightSaveTariffCart}
                            />
                          )}
                      ...
                      {!preview.can_purchase &&
                        (preview.missing_amount_kopeks > 0 ? (
                          <InsufficientBalancePrompt
                            ...
                            compact
                            onBeforeTopUp={preflightSaveClassicCart}
                          />
