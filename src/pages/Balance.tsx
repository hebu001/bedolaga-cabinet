...
export default function Balance() {
  ...
  const [topUpInitialAmount, setTopUpInitialAmount] = useState<string | undefined>(undefined);
  ...
  // Auto-open the top-up modal with a pre-filled amount when arriving from
  // ?amount=X (e.g. InsufficientBalancePrompt → /balance/top-up?amount=150).
  const autoTopUpHandledRef = useRef(false);
  useEffect(() => {
    if (autoTopUpHandledRef.current) return;
    const amountParam = searchParams.get('amount');
    if (!amountParam) return;
    const parsed = parseFloat(amountParam);
    if (!Number.isFinite(parsed) || parsed <= 0) return;
    autoTopUpHandledRef.current = true;
    setTopUpInitialAmount(String(Math.ceil(parsed)));
    setShowTopUp(true);
    // Strip the param so reopening Balance doesn't re-trigger the modal.
    const next = new URLSearchParams(searchParams);
    next.delete('amount');
    navigate(`/balance${next.toString() ? `?${next.toString()}` : ''}`, { replace: true });
  }, [searchParams, navigate]);
